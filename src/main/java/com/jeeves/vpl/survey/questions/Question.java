...
public abstract class Question extends ViewElement<FirebaseQuestion> implements SurveyElement {
	public static Question create(FirebaseQuestion question) {
		String classname = question.gettype();
		try {
			return (Question) Class.forName(classname).getConstructor(FirebaseQuestion.class).newInstance(question); 
		} catch (Exception e) {
			System.exit(1);
		}
		return null;
	...
	protected StringProperty questionTextProperty;
	//HBox buttonBox;
	...
	public Question() throws InstantiationException, IllegalAccessException {
		super(FirebaseQuestion.class.newInstance(),FirebaseQuestion.class);
		}

	public Question(FirebaseQuestion model) {
...
//	}
	...
	public void addListeners() {
		...
		EventHandler<MouseEvent> overHandler = arg0 ->
			showEdit();
		this.addEventHandler(MouseEvent.MOUSE_ENTERED, overHandler);
		EventHandler<MouseEvent> outHandler = arg0 ->
			hideEdit();
		this.addEventHandler(MouseEvent.MOUSE_EXITED, outHandler);
